...
- Nested regular files are included under recursive total manifest closure when
  ...
  rejected. Under active rc6.4 (RC6-R4 retained), every nested regular file must also
  resolve to exactly one registered typed nested evidence class; listed+hashed
  is not sufficient authorization. Empty directories are rejected for active
  rc6.4 before manifest generation and again during validator filesystem closure.
...
- Deviation transition (RC6-R5 / R5-D1): distinct Host preliminary vs final Witness
  ...
  aggregate severity / canonicality / `final_machine_ceiling`
- Machine ceiling (RC6-R5 / R5-C3): validator independently recomputes the ceiling;
...
Active candidate-native identity (Repository Owner): **WF-FC-04** / **`weaver-forge-fc-04`**
Active candidate name: **Weaver Forge Future Candidate 4**. Mapping **WF-FC-04 -> weaver-forge-fc-04**.
Historical Future Candidate mappings: **WF-FC-01 -> weaver-forge-fc-01**; **WF-FC-02 -> weaver-forge-fc-02**; **WF-FC-03 -> weaver-forge-fc-03**.
Active package surfaces use WF-FC-04 / `weaver-forge-fc-04`. RC6/RC7/RC8 remain immutable **historical**
...
| `PACKAGE_TAG_EXPECTED` | alias of active Future Candidate tag (`weaver-forge-fc-04`) |
...
|----------------------------|--------------|
| `WF-FC-04` | `weaver-forge-fc-04` (**active**) |
| `WF-FC-03` | `weaver-forge-fc-03` (historical) |
...
| `1.0.0-rc5` | `grok-build-witness-v1.0.0-rc5` (historical) |
| `1.0.0-rc5-phase4-s3-fixture` | `grok-build-witness-v1.0.0-rc5` |
| `1.0.0-rc4` | `grok-build-witness-v1.0.0-rc4` |
| any other / unknown / absent / malformed value | **fail closed** (unsupported) |

Rules:

- Active Future Candidate package version `WF-FC-04` requires tag `weaver-forge-fc-04`.
- Historical Future Candidate versions `WF-FC-01` / `WF-FC-02` / `WF-FC-03` require their exact historical tags.
...
- Absent, empty, malformed, contradictory, or unsupported package versions fail closed.
- Requested tag, canonical tag, and verdict `package_tag` must equal the expected tag for the declared version.
- Identity record and final binding must cross-file agree on package version / canonical tag.
- Future tag object ID and peeled commit are runtime-resolved; do not fabricate IDs.
