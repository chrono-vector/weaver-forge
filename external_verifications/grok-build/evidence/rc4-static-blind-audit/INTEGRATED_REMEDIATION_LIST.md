...
| Status | **PARTIAL (Phase 3C container-side + Phase 3D host-side + Phase 3E POST_BUILD + Phase 4-S2/S3):** `RC4B-011` remains **OPEN** with container-side implementation recorded; Phase 4 records schema/validator progress only. `RC4B-024` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** for finalized S2-shaped terminal packages (NOT_APPLICABLE replaces surviving NOT_REACHED placeholders under S2 rules); historical S1 fixtures may retain NOT_REACHED via explicit compatibility. **Not CLOSED.** `RC4B-012` remains **OPEN** — Phase 3D/3E/3F-B recorded; Phase 4 completeness/inventory progress recorded without closing broader invalid-result gaps. `RC4B-029` remains **OPEN** with prior host/container progress plus Phase 4 inventory/completeness progress recorded. **Not CLOSED.** |
...
| Status | **PARTIAL (Phase 3E POST_BUILD + Phase 4-S2/S3):** `RC4B-016` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** for broad schema/writer/template/fixture/validator alignment under the active S2 register (POST_BUILD exactness retained; S2 package-identity / DEVIATIONS / HOST_RUN_METADATA / terminal schemas activated). **Not CLOSED.** `RC4B-006` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** for preliminary/final DEVIATIONS structural alignment under mode-specific S2 schemas (severity/ceiling lifecycle RC4B-007/034–036 remain later). **Not CLOSED.** `RC4B-021` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** for active S2 exact allowed-key enforcement on S2-shaped evidence (unknown fields rejected; historical S1 compatibility remains explicit). **Not CLOSED.** |
...
| Status | **PARTIAL (Phase 3B contract + Phase 3D host tuple ingestion + Phase 3E POST_BUILD + Phase 3F-A validator prerequisites + Phase 3F-B host validator gating + Phase 3G generator/integration):** `RC4B-014` and `RC4B-023` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** — Phase 3D host parses the complete required tuple (not only `outcome=`), preserves valid container outcomes, and records host-owned ingestion; Phase 3E adds host-owned complete truthful POST_BUILD. **Not CLOSED.** `RC4B-015` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** — Phase 3E centralizes host POST_BUILD finalization with `status=OK` iff `post_build_integrity_ok=yes` and complete FAILED records on finalized failure paths; pending integration, fixed candidate, and reaudit. **Not CLOSED.** `RC4B-022` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** — Phase 3F-A removes all `determine_outcome` inference fallbacks and requires an explicit valid authoritative `outcome=`; Phase 3G-B adds generator-backed integrated disagreement/no-inference evidence; pending fixed candidate and reaudit. **Not CLOSED.** `RC4B-017` remains **OPEN** with **PARTIALLY_IMPLEMENTED** host-preliminary progress: Phase 3F-A enforces only the automatable host-preliminary structural subset (`POST_BUILD` status/ok/source/lock fields + matching `HOST_OUTCOME` host statuses) without requiring `evidence_inventory_complete=yes`; full four-yes / final success eligibility remains outstanding. **Not CLOSED.** `RC4B-013` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** — Phase 3F-B host invokes repository validator in `--host-preliminary` mode after preliminary manifest finalization, records host-owned `VALIDATOR_RESULT.txt` outside `EVIDENCE_DIR`, and gates host exit 0 on validator process exit 0 plus exact `STRUCTURAL VALIDATION: PASS` plus the adjudicated automated Docker/outcome/HOST_OUTCOME/POST_BUILD conditions; Phase 3G-B adds generator-backed real-validator primary integration evidence; pending fixed candidate and reaudit. **Not CLOSED.** `RC4B-019` remains **OPEN** with Phase 3E host POST_BUILD progress recorded (schema-aligned POST_BUILD does not close broader behavioral/generator coverage). |
...
| Status | **PARTIAL (Phase 4-S3 on main):** recursive inventory totality, final-manifest cryptographic closure for S2-shaped packages (no auxiliary exemption), validator captures/`VALIDATOR_RESULT` outside `EVIDENCE_DIR`, and non-circular completeness sequencing are **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT**. `RC4B-020`, `RC4B-026`, `RC4B-027`, and `RC4B-028` advance to **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT**. Historical S1 fixture compatibility retains explicit unlisted closed-aux acceptance. **Not CLOSED.** Final closure requires fixed candidate and repeat static audit. |
...
| Status | **PARTIAL (Phase 2A + Phase 2B + Phase 3G-B):** `RC4B-009` remains **OPEN** — Phase 2A host-safety subset for RC4B-004/005/008 retained; Phase 2B adds a **subset** of required behavioral coverage (mount-plan isolation / writable-alias rejection / argv boundary / pre-post source integrity only). Phase 3G-B adds generator-backed automated preliminary integration coverage on `main` but does not close broader mount/manifest/PASS/correction behavioral gaps. `RC4B-018` → **IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT** — Phase 3G-B implements Pi-adjudicated generator-backed integrated scenarios exercising committed sourced writers, real local `--host-preliminary` validator primary success integration, invalid BUILD_EXIT / disagreement / host+POST_BUILD fail-closed paths, stale/spoof/mixed-run rejection, evidence immutability, and limited full-main fail-closed smoke; pending fixed candidate and reaudit. **Not CLOSED.** `RC4B-040` → automated preliminary package subset is **implemented/integration-evidenced on main pending reaudit** (Phase 3G-B); final manual Witness submission / Independent Witness lifecycle remains later and the blocker is **not CLOSED**. `RC4B-019` remains **OPEN**. |
...
| Remediation implementation begun | Phase 1 documentation/status-truthfulness (RC4B-001/002/003); Phase 2A host preflight (RC4B-004/008 REMEDIATED_ON_MAIN_PENDING_REAUDIT after Cursor + Pi 18/18; RC4B-005/009 remain OPEN with notes; **not CLOSED**); Phase 2B source-mount isolation (RC4B-010 REMEDIATED_ON_MAIN_PENDING_REAUDIT after Cursor + Pi corrected-suite PASS; **not CLOSED**); Phase 3B authoritative outcome ownership contract (RC4B-022 advanced by Phase 3F-A to IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT; RC4B-014/023 advanced by Phase 3D); Phase 3C container terminal finalization (RC4B-011/024/029 remain OPEN with container-side implementation recorded); Phase 3D host outcome ingestion (RC4B-014/023 → IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT; RC4B-012/029 remain OPEN with host-side implementation recorded; **not CLOSED**); Phase 3E host POST_BUILD integrity (RC4B-015 → IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT; RC4B-016 POST_BUILD field-set subset only — not fully remediated / not CLOSED; RC4B-012/029/019 remain OPEN with Phase 3E progress recorded; **not CLOSED**); Phase 3F-A validator prerequisites (RC4B-022 → IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT; RC4B-017 OPEN with automatable host-preliminary subset only; **not CLOSED**); Phase 3F-B host validator gating (RC4B-013 → IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT; RC4B-017 remains only partially advanced; RC4B-012/029/019 remain OPEN; **not CLOSED**); Phase 3G-A/B generator/harness + integrated scenarios (RC4B-018 → IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT; RC4B-040 automated preliminary subset implemented/integration-evidenced pending reaudit — final manual-submission later; **not CLOSED**; RC4B-013/022 retain IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT with Phase 3G evidence; RC4B-009/012/017/019/029 remain OPEN) |
| Technical implementation remediation (scripts/validator/tests/templates/execution controls) begun | **Yes — Phase 2A host preflight + Phase 2B mount isolation + Phase 3C container terminal finalization + Phase 3D host outcome ingestion + Phase 3E host POST_BUILD integrity / schema alignment + Phase 3F-A validator explicit-outcome / HOST_OUTCOME structural validation / host-preliminary RC4B-017 subset + Phase 3F-B host validator invocation / host-owned VALIDATOR_RESULT / validator-gated host exit + Phase 3G generator framework and integrated sourced-writer / real-validator scenarios** (Phase 3B is contract/docs/tests only; Phase 3G does not modify runtime scripts, validator inference removal beyond Phase 3F-A, outcome-contract JSON, CLAIM_REGISTER, or historical Phase 3B/3C/3D/3E/3F notes) |
| Blockers CLOSED | **0** |
| Blockers REMEDIATED_ON_MAIN_PENDING_REAUDIT | RC4B-001, RC4B-002, RC4B-003, RC4B-004, RC4B-008, RC4B-010 |
| Blockers IMPLEMENTED_ON_MAIN_PENDING_INTEGRATION_AND_REAUDIT | RC4B-005, RC4B-006, RC4B-013, RC4B-014, RC4B-015, RC4B-016, RC4B-018, RC4B-020, RC4B-021, RC4B-022, RC4B-023, RC4B-024, RC4B-026, RC4B-027, RC4B-028 |
| Blockers CONTRACT_DEFINED_ON_MAIN_IMPLEMENTATION_PENDING | *(none after Phase 3F-A)* |
| Blockers OPEN_PENDING_CROSS_ENV_CORROBORATION | *(none)* |
| Independent Witness / C-014 advanced | **No** / `NOT_STARTED` |
| Phase 4-S3 progress (not CLOSED) | Manifest totality + completeness state machine + rc5 fixture families + full 2A–4-S3 regression on `main` pending Pi conformance/commit; RC4B-009/011/012/017/019/025/029/040 record Phase 4 progress only and remain not CLOSED; RC4B-030–039 remain OPEN; RC4B-013/014/015/018/022/023 prior pending-reaudit statuses preserved |
